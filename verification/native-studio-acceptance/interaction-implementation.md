...
## Integrated test failure diagnosis

The integration run retained in `verification/native-studio-alpha/checks/acceptance-native-tests.txt` completed with exit 101: 139 passed, 6 failed. Both native camera tests passed (legacy residual-tail reproduction and 12,000 event-order cycles), as did mixed-DPI and navigation tests. Two layout failures were assigned back to the Worlds owner. The four view-intent failures exposed old serial-read expectations and a real cancellation race: cancellation discarded the latest current-view request before its completion. Cancellation now retains and reissues that exact desired lens against the immutable current revision; an already completed read remains displayed. Routing regressions cover both completion orders, preparation-to-review supersession, cancelled preparation retaining current reads, and candidate-only deferral until cancellation acknowledgement. These revised checks are awaiting integration execution.

Local verification after this fix: `rustfmt --edition 2024 crates/studio-native/src/updates.rs crates/studio-native/src/view_intent_tests.rs` exit 0 (no output); `git diff --check` exit 0 (line-ending notices only).
