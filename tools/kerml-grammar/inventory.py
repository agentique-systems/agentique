...
def main():
    ...
    inventory = {
        ...
        "productions": [{"production": name, "count": len(items), "documents": sorted({i["document"] for i in items}),
                         ...
                         "parser_support": "bounded authored subset" if name in EXISTING else "not implemented in Rust frontend",
                         "ast_cst_support": "bounded authored subset" if name in EXISTING else "reference inventory tree only",
