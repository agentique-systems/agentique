...
#[test]
fn complete_inventory_separates_runtime_readiness_from_conformance() {
    ...
    assert_eq!(audit["registration_attempted"], true);
    assert!(audit["translation_error"].is_null() && audit["registration_error"].is_null());
    assert!(audit["runtime_errors"].as_array().unwrap().is_empty());
    assert_eq!(audit["conformance_result"], "errors");
    assert!(audit["conformance"].as_array().unwrap().iter().any(|d| {
        d["source"]["external_id"] == "Systems-Flows-A_flowDefinition_definedFlow-definedFlow"
            && d["disposition"]["kind"] == "reviewed-baseline-anomaly"
    }));
    assert!(!audit["association_shapes"].as_object().unwrap().is_empty());
...
#[test]
fn readiness_check_requires_all_generated_outputs_and_never_writes_missing_files() {
    let directory = tempfile::tempdir().unwrap();
    for baseline in ["kerml-1.0", "sysml-2.0"] {
        ...
        assert!(!result.status.success());
        assert!(
            String::from_utf8_lossy(&result.stderr).contains("The system cannot find")
