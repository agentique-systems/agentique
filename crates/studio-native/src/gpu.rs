...
pub fn install(cc: &eframe::CreationContext<'_>) -> Result<(), String> {
    let state = cc.wgpu_render_state.as_ref().ok_or("Native Studio requires a wgpu adapter")?;
    let device = &state.device;
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor { label: Some("Agentique retained scene"), source: wgpu::ShaderSource::Wgsl(include_str!("scene.wgsl").into()) });
    ...
    let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        ...
        fragment: Some(wgpu::FragmentState { module: &shader, entry_point: Some("fragment"), compilation_options: Default::default(), targets: &[Some(wgpu::ColorTargetState { format: state.target_format, blend: Some(wgpu::BlendState::PREMULTIPLIED_ALPHA_BLENDING), write_mask: wgpu::ColorWrites::ALL })] }),
        primitive: Default::default(), depth_stencil: None, multisample: Default::default(), multiview: None, cache: None,
    });
    let uniform = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("Scene camera uniform"), contents: bytemuck::cast_slice(&[0.0f32; 8]), usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST });
    let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor { label: Some("Scene camera binding"), layout: &layout, entries: &[wgpu::BindGroupEntry { binding: 0, resource: uniform.as_entire_binding() }] });
    let instances = device.create_buffer(&wgpu::BufferDescriptor { label: Some("Retained scene instances"), size: 64, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
    state.renderer.write().callback_resources.insert(Resources { pipeline, bind_group, uniform, instances, capacity: 64, ranges: [0..0,0..0,0..0,0..0], key: None });
    Ok(())
}
...
impl CallbackTrait for SceneCallback {
    fn prepare(&self, device: &wgpu::Device, queue: &wgpu::Queue, _screen: &ScreenDescriptor, _encoder: &mut wgpu::CommandEncoder, resources: &mut CallbackResources) -> Vec<wgpu::CommandBuffer> {
        let renderer = resources.get_mut::<Resources>().expect("scene renderer installed");
        queue.write_buffer(&renderer.uniform, 0, bytemuck::cast_slice(&self.camera));
        if renderer.key != Some(self.batch.key) {
            let started = Instant::now();
            let lists = [&self.batch.containers, &self.batch.edges, &self.batch.nodes, &self.batch.overlays];
            ...
            if bytes.len() > renderer.capacity {
                ...
                renderer.instances = device.create_buffer(&wgpu::BufferDescriptor { label: Some("Retained scene instances"), size: renderer.capacity as u64, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
            }
            if !bytes.is_empty() { queue.write_buffer(&renderer.instances, 0, bytes); }
            let mut offset = 0u32;
            ...
            if let Ok(mut stats) = self.stats.lock() {
                ...
                stats.uploaded_bytes = bytes.len(); stats.uploads += 1; stats.instances = all.len();
                stats.draw_calls = renderer.ranges.iter().filter(|range| !range.is_empty()).count();
            }
        }
        Vec::new()
    }
    fn paint(&self, _info: egui::PaintCallbackInfo, pass: &mut wgpu::RenderPass<'static>, resources: &CallbackResources) {
        let renderer = resources.get::<Resources>().expect("scene renderer installed");
        pass.set_pipeline(&renderer.pipeline);
        pass.set_bind_group(0, &renderer.bind_group, &[]);
        pass.set_vertex_buffer(0, renderer.instances.slice(..));
        for range in &renderer.ranges {
            if !range.is_empty() { pass.draw(0..6, range.clone()); }
        }
    }
}
