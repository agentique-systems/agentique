...
impl KerMlQueries<'_> {
    ...
    fn operational_redefinition_target(
        ...
        if name.absolute {
            ...
            let mut generals = self.supertypes(owning_type);
            // Resolution precedes optional 8.4.2 removal of redundant implied
            // specializations. Their namespaces remain resolution scopes: a
            // renamed redefinition in an explicit general can hide a name that
            // is independently available through a required library general.
            // This is the v2 pilot `gen` / getImplicitGeneralTypesFor phase,
            // before removeUnnecessaryImplicitGeneralTypes. It does not add
            // aliases or search lexical parents of the general namespaces.
            if !self.context().options.exclude_implied {
                let required = self.required_library_bases(owning_type);
                generals.value.extend(required.value.iter().copied());
                generals.merge(required);
                generals.value.sort();
                generals.value.dedup();
            }
            let mut candidates = BTreeSet::new();
