# Language core foundation review ? completion v2, 2026-09-18

**LANGUAGE CORE STRUCTURAL FOUNDATION INCOMPLETE ? DO NOT PROCEED**

Base: `a93f3e94630abc64a3ea16ede27abc22b1dd2d58`; clean initial worktree;
branch `foundation/language-core-completion-v2`.

[ADR 0010](adr/0010-property-redefinition-validity.md) supersedes the acquisition
requirement in ADR 0009. All three requested OMG resolutions were acquired and
inspected. The remaining Gate 0 conflict concerns the independent final Property
inherited-feature constraint and the absence of either relevant inheritance
relation for the exact pinned SysML edge. The four ownership cases and competing
interpretations are explicit. No replacement invariant is accepted or implemented.
Gates 1?11 remain uncompleted; ordinary generic capability gaps are not this stop's
reason. The earlier review is preserved below and as exact original bytes in
[the new verification record](../verification/language-core-completion-v2/original-foundation-review.md).

## Current answers to the same 13 questions

| Question | Answer and machine-verifiable evidence |
| --- | --- |
| 1. Every KerML metaclass/property/association represented at runtime? | **No.** [Complete runtime command](../verification/language-core-completion-v2/stage0/results.json) exits 1 before registry construction. |
| 2. Every SysML metaclass/property/association represented at runtime? | **No.** The same record shows the combined gate exits 1; no runtime crate exists. |
| 3. Cross-metamodel references preserved exactly? | **Raw manifests only.** [Independent XML check](../verification/language-core-completion-v2/focused/independent.txt) verifies complete source identities/references. Runtime preservation remains unproven. |
| 4. Remaining blanket exceptions? | No new exception or rule change. [Preservation capture/checker](../verification/language-core-completion-v2/preservation.mjs) checks all runtime/generator source against fetched main. |
| 5. Name-based descriptor hacks? | None introduced; runtime/generator source and UUID mapping are preserved by that check and the independent XML check. |
| 6. Source anomalies silently normalized? | No. Original XMI and all five anomaly entries are protected; [source evidence](../verification/language-core-completion-v2/evidence/property-authority.json) retains the exact failing redefinition and ancestry. |
| 7. Normative primitives narrowed? | No; complete translation still refuses Integer/Real, as recorded in both [runtime logs](../verification/language-core-completion-v2/stage0/results.json). Representation remains unimplemented. |
| 8. Association occurrences without duplicate writable truth? | Existing supported shapes only. [Kernel tests](../verification/language-core-completion-v2/focused/kernel.txt) pass; no full storage-shape certification exists. |
| 9. Derived/uncomputed states distinguishable? | Existing partial contract only. Kernel tests pass; the required complete five-state contract remains unimplemented. |
| 10. Immutable snapshots sole canonical declared state? | Yes within the unchanged implementation, protected by source preservation and exercised by the kernel suite. |
| 11. Typed views without duplicate semantic data? | Existing bounded views only; their bytes are protected. Complete language views do not exist. |
| 12. Existing queries safe against full registry? | **Not established.** Neither complete registry constructs; the exact bounded-context restriction is unchanged. |
| 13. Known gaps explicitly recorded? | Yes: ADR 0010, the complete audits and [generation-2 coverage](../standards/v2-coverage.json). Recording gaps is not completion. |

The [new evidence README](../verification/language-core-completion-v2/README.md)
links actual commands/exits. The candidate ownership matrix is diagnostic
